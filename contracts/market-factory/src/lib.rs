...
mod tests {
    ...
    fn setup() -> (Env, Address, Address, Address, Address) {
        ...
        (env, admin, creator, oracle, client_addr)
    }

    #[test]
    fn test_create_market_no_pools() {
        let (env, admin, creator, oracle, contract_id) = setup();
        ...
        let id = client.create_market(
            &creator,
            &String::from_str(&env, "Will ETH hit $5k?"),
            &String::from_str(&env, "ETH price market"),
            &Category::Crypto,
            &oracle,
        ...
        // Market struct no longer has pool fields
        assert_eq!(market.min_bet, 1_000_000i128);
        assert_eq!(market.trading_fee_bps, 200u32);
    }
    ...
    fn test_full_lifecycle() {
        ...
        client.lock_market(&id);
        assert_eq!(client.get_market(&id).status, MarketStatus::Locked);

        client.request_oracle(&id);
        assert_eq!(client.get_market(&id).status, MarketStatus::AwaitingOracle);
    ...
    fn test_pause_resume() {
        ...
        client.pause_trading(&id);
        assert_eq!(client.get_market(&id).status, MarketStatus::TradingPaused);
        client.resume_trading(&id);
        assert_eq!(client.get_market(&id).status, MarketStatus::Open);
    }
}
